...
useEffect(() => {
    settotalQuantity(subworks.reduce((acc, subwork) => acc + parseInt(subwork.quantity || 0), 0));
                ...
                const updatedSubworks = fetchedSubworks.map((subwork) => {
                    const updatedQuantity =
                        unitsDefault.SFT !== 0
                            ...
                            : subwork.length * subwork.breadth * subwork.depth * subwork.number;
                        sum+=updatedQuantity;
                ...
                const updatedReductions = fetchedReductions.map((redeuction) => {
                    const updatedQuantity =
                        unitsDefault.SFT !== 0
                            ...
                            : redeuction.length * redeuction.breadth * redeuction.depth * redeuction.number;
                        sum+=updatedQuantity;
    ...
    const handleDelete = async (detailId) => {
        try {
            ...
            const response = await axios.delete(`${import.meta.env.VITE_API_URL}/deletesubworkfield/${swid}/${detailId}`, {
                headers: {
                    Authorization: `Bearer ${Token}`,
                },
            });
            ...
            console.error('Error deleting subwork detail:', error.response?.data?.message || error.message);
        }
    };
    const handleDeleteReduction = async (detailId) => {
        try {
            let swid = wid;
            // console.log(`Deleting detail with ID: ${detailId} from Subwork ID: ${swid}`);
            const response = await axios.delete(`${import.meta.env.VITE_API_URL}/deletesubworkreduction/${swid}/${detailId}`, {
                headers: {
                    Authorization: `Bearer ${Token}`,
                },
            });
    ...
    return (
        <div className="p-4">
            <div className="flex justify-between flex-wrap">
                ...
                <div className="flex flex-col space-y-4 justify-end mb-2 ">
                    <div className="flex space-x-1">
                        <label htmlFor="SFT" className="text-sm font-medium text-gray-700 dark:text-gray-300 w-16">₹ / SFT:</label>
                        <input
                            type="text"
                            id="SFT"
                            value={units.SFT}
                    ...
                    <div className="flex items-center space-x-1">
                        <label htmlFor="CFT" className="text-sm font-medium text-gray-700 dark:text-gray-300 w-16">₹ / CFT:</label>
                        <input
                            type="text"
                            id="CFT"
                            value={units.CFT}
            ...
            {showModal && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                    <div className="bg-white dark:bg-gray-800 p-6 rounded shadow-lg w-96 max-h-[90vh] overflow-y-auto">
                        <h3 className="text-lg font-bold mb-4 text-black dark:text-white">Add Subwork</h3>
                        <form>
                        ...
                        {newSubworks.length > 0 && (
                            <div className="mt-4">
                                <h4 className="font-bold text-black dark:text-white">Batch Preview:</h4>
                                    ...
                                    <ul>
                                        {newSubworks.map((subwork, index) => (
                                            ...
                                            >
                                                ...
                                                <button
                                                    className="px-2 py-1 bg-red-500 dark:bg-red-600 text-white text-sm rounded"
                    ...
                    <tbody>
                        {subworks.map((subwork, index) => (
                            <tr key={subwork.id}>
                                ...
                                <td className="border p-2">{subwork.name}</td>
                                <td className="border p-2">{subwork.number}</td>
                                <td className="border p-2">{subwork.length}</td>
                                <td className="border p-2">{subwork.breadth}</td>
                                <td className="border p-2">{subwork.depth}</td>
                                {/* <td className="border p-2">
                                {new Intl.NumberFormat('en-IN').format(
                                    subwork.quantity)
                                    }</td> */}
                                <td className="border p-2">{
                                    units.SFT!=0 ? (subwork.length * subwork.breadth * subwork.number ) : (subwork.length * subwork.breadth * subwork.depth * subwork.number )
                                    }</td>
                                
                                {/* <td className="border p-2">{subwork.totalval}</td> */}
                                <td className="border p-2 flex flex-col sm:flex-row sm:space-x-2">
                                    <button
                                        className="px-2 py-1 bg-blue-500 text-white rounded mb-1 sm:mb-0"
                        ...
                        <tr>
                            ...
                            <td className=""> </td>
                            <td className=""> </td>
                            <td className="border p-2 font-bold"> total quantity</td>
                ...
                <div className=" mt-[2vw] overflow-x-auto">
                    ...
                    <tbody>
                        {reductions.map((reduction, index) => (
                            <tr key={reduction.id}>
                                <td className="border p-2">{index + 1}</td>
                                <td className="border p-2">{reduction.name}</td>
                                <td className="border p-2">{reduction.number}</td>
                                <td className="border p-2">{reduction.length}</td>
                                <td className="border p-2">{reduction.breadth}</td>
                                <td className="border p-2">{reduction.depth}</td>
                                {/* <td className="border p-2">
                                {new Intl.NumberFormat('en-IN').format(
                                    subwork.quantity)
                                    }</td> */}
                                <td className="border p-2">{
                                    units.SFT!=0 ? (reduction.length * reduction.breadth * reduction.number ) : (reduction.length * reduction.breadth * reduction.depth * reduction.number )
                                    }</td>
                        ...
                        <tr>
                            ...
                            <td className=""> </td>
                            <td className=""> </td>
                            <td className="border p-2 font-bold"> total Reduction quantity</td>
                            <td className="border p-2 font-bold "> {
                             new Intl.NumberFormat('en-IN').format(
                            RtotalQuantity)}</td>
                            <td className=""> </td>  
                        ...
                        <tr>
                            ...
                            <td className=""> </td>
                            <td className=""> </td>
                            <td className="border p-2 font-bold"> Amount total</td>
            ...
            {showModal && editingSubwork && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                    <div className="bg-white dark:bg-gray-800 p-6 rounded shadow-lg w-96 max-h-[90vh] overflow-y-auto">
                        <h3 className="text-lg font-bold mb-4 text-black dark:text-white">Edit Subwork</h3>
                        <form>
                            {["name", "number", "length", "breadth", "depth",].map((field) => (
                                <div key={field} className="mb-4">
                                    <label className="block text-sm font-medium mb-1 text-black dark:text-gray-300">
